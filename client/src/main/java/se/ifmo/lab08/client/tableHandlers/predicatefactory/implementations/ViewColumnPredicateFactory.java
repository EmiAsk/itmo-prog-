...
public class ViewColumnPredicateFactory extends PredicateFactory {
    @Override
    ...
    private Predicate<Flat> createMorePredicate(String value) {
        return (band) -> band.getView().toString().compareTo(value) > 0;
    }
    ...
    private Predicate<Flat> createEqualPredicate(String value) {
        return (band) -> band.getView().toString().equalsIgnoreCase(value);
    }
