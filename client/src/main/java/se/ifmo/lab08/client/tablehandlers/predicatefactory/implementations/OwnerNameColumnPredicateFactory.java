...
public class OwnerNameColumnPredicateFactory extends PredicateFactory {
    @Override
    ...
    private Predicate<Flat> createMorePredicate(String value) {
        return (band) -> band.getOwner().getUsername().compareTo(value) > 0;
    }
    ...
    private Predicate<Flat> createEqualPredicate(String value) {
        return (band) -> band.getOwner().getUsername().equals(value);
    }
